...
const DistributionMap = (props: any) => {
  ...
  const xAxis = useMemo(() => {
    return d3.axisBottom(xScale).ticks(0);
  }, [xScale]);
  const yAxis = useMemo(() => {
    return d3.axisLeft(yScale).ticks(5).tickSize(6).tickPadding(3);
  }, [yScale]);
  ...
  useEffect(() => {
    ...
    if (points.length > 0) {
      const legend = svg.append("g").attr("class", "lineGraph-legend");
      legend
        .append("text")
        .attr("x", marginX)
        .attr("y", marginY + chartHeight + 20)
      ...
      svg
        .append("g")
        .attr("class", "lineGraphBrush")
        .call(brush)
        .call(brush.move, [xScale(0) + marginX, xScale(1) + marginX]);
    }
